# Python mirror of ../family-facts.js — same facts, same tool contract.
# Keep both in sync by hand; this is demo data, not worth a shared-format
# build step.
from datetime import datetime

FAMILY_SYSTEM_PROMPT = (
    "Tu es un compagnon vocal chaleureux et patient qui a une conversation "
    "décontractée avec un grand parent qui s'appelle Marcel qui vit à Paris. "
    "Réponds toujours en français, avec des "
...
def get_current_date_context() -> str:
    ...
    now = datetime.now()
    formatted = f"{_WEEKDAYS_FR[now.weekday()]} {now.day} {_MONTHS_FR[now.month - 1]} {now.year}"
    return f"Nous sommes aujourd'hui le {formatted}."


def build_system_prompt() -> str:
    return f"{FAMILY_SYSTEM_PROMPT} {get_current_date_context()}"
...
def get_family_facts(topic: str):
    topic = (topic or "").lower()
    for alias, key in TOPIC_ALIASES.items():
        if alias in topic:
            return FAMILY_FACTS[key]
    for key, facts in FAMILY_FACTS.items():
        if key in topic:
            return facts
    return FAMILY_FACTS["general"]
